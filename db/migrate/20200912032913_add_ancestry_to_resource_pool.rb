class AddAncestryToResourcePool < ActiveRecord::Migration[5.2]
  ...
  def up
    ...
    say_with_time("set vm ancestry from existing genealogy relationship resource information") do
      transfer_relationships_to_ancestry(ResourcePool, 'ResourcePool', 'genealogy')
    end
  ...
  def down
    say_with_time("create relationship records from vm ancestry") do
      create_relationships_from_ancestry(ResourcePool.all, Relationship, 'ResourcePool', 'genealogy')
    end
